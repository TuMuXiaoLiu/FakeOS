	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511){//键盘
				//将按键编码转为字符编码
				if (i < 0x80 + 256){
					if (key_shift == 0){ s[0] = keytable0[i - 256]; } else { s[0] = keytable1[i - 256]; } } else { s[0] = 0; }
				//当输入的字符为英文字母时
				if ('A' <= s[0] && s[0] <= 'Z'){
					//将大写字母转为小写字母
...
void file_readfat(int *fat, unsigned char *img){//将磁盘印象中的FAT解压缩
	int i, j = 0;
	for (i = 0; i < 2880; i +=2){
...
void file_loadfile(int clustno, int size, char *buf, int *fat, char *img){
	...
	for (; ; ){
		...
		for (i = 0; i < 512; i++){ buf[i] = img[clustno * 512 +1]; }
		size -=512;
		buf +=512;
		clustno = fat[clustno];
	}
